...
static bool UpdatePlayer(int playerTurn)
{
    ...
    if (GetMousePosition().y <= player[playerTurn].position.y)
    {
        ...
        if (player[playerTurn].isLeftTeam && GetMousePosition().x >= player[playerTurn].position.x)
        {
            ...
            player[playerTurn].aimingPower = sqrtf(mypow(player[playerTurn].position.x - GetMousePosition().x) + mypow(player[playerTurn].position.y - GetMousePosition().y));
            // Calculates the angle via arcsin
            player[playerTurn].aimingAngle = asinf((player[playerTurn].position.y - GetMousePosition().y)/player[playerTurn].aimingPower)*RAD2DEG;
...
static bool UpdateBall(int playerTurn)
{
    ...
    if (!ball.active)
    {
        if (player[playerTurn].isLeftTeam)
        {
            ball.speed.x = cosf(player[playerTurn].previousAngle*DEG2RAD)*player[playerTurn].previousPower*3/DELTA_FPS;
            ball.speed.y = -sinf(player[playerTurn].previousAngle*DEG2RAD)*player[playerTurn].previousPower*3/DELTA_FPS;
            ball.active = true;
        }
        else
        {
